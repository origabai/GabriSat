...
class SAT:
    ...
    def check_clause_satisfaction(
        self, clause: Clause, interpretation: list[bool]
    ) -> bool:
        ...
        variable_satisfaction.extend(
            [not interpretation[i] for i in clause.neg_variables]
        )
    ...
    def check_interpretation_satisfaction(self, interpretation: list[bool]) -> bool:
        clauses_satisfaction = [
            self.check_clause_satisfaction(clause, interpretation)
            for clause in self.clauses
        ]
    ...
    def recursive_backtrack_solve(
        ...
        if len(singelton_clauses) > 0:  # there are singelton clauses
            ...
            # unsatisfied = unsatisfied_copy.copy()
            # singelton_clauses = singelton_clauses_copy.copy()
            # assert(interpretation == interpretation_copy)

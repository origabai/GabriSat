...
int score_literal_sudoku(literal l) {
    int score = 0;
    score = -l.ind;
    return score;
}
...
bool less_literal_sudoku(literal l1, literal l2) {
    ...
    if (score_literal_sudoku(l1) >= score_literal_sudoku(l2)) return true;
    return false;
...
class SATHandler_Sudoku : public SATHandlingDS{
    ...
    PersistentGeneralSegmentTreeDS<literal, literal::literal_e, less_literal_sudoku> minqryds;
    ...
    void initialize(int N, std::vector<SATClause> C) override {
        ...
        minqryds = PersistentGeneralSegmentTreeDS<literal, literal::literal_e, less_literal_sudoku>(num_variables);
...
class SudokuSolver : public AbstractThreadedSolver {
    public:
    SudokuSolver(int num_variables) : AbstractThreadedSolver(num_variables, new SATHandler_Sudoku()) {}
};